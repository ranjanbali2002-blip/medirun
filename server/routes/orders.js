  ...
  try {
    ...
    const { rows } = await client.query(
      `INSERT INTO orders (id,user_id,total,delivery_fee,delivery_address,delivery_lat,delivery_lon,
        delivery_distance,items,status,payment_status,prescription_data,requires_prescription,delivery_otp)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9,'pending','unpaid',$10,$11,$12) RETURNING *`,
      [orderId, req.user.id, total, delivery_fee, delivery_address, delivery_lat, delivery_lon,
       delivery_distance, medicines.length, prescription_data || null, requires_prescription || false, deliveryOtp]
    );
...
// Rider submits delivery OTP to confirm handoff
router.post("/:id/verify-delivery", requireAuth, requireRole("rider"), async (req, res) => {
  try {
    const { otp } = req.body;
    const { rows } = await pool.query(
      "SELECT delivery_otp, status FROM orders WHERE id=$1", [req.params.id]
    );
    if (!rows[0]) return res.status(404).json({ error: "Order not found" });
    if (rows[0].status === "delivered") return res.status(400).json({ error: "Already delivered" });
    if (rows[0].delivery_otp !== otp) return res.status(400).json({ error: "Wrong OTP" });
    const { rows: updated } = await pool.query(
      "UPDATE orders SET status='delivered' WHERE id=$1 RETURNING *", [req.params.id]
    );
    res.json(updated[0]);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
